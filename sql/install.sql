...
SELECT 
    ...
    g.genre_name, 
    CASE 
        WHEN e.festival_year = (in_year) THEN 'Yes' 
        ELSE 'No' 
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_count FROM buyer_queue WHERE status = 'pending';
    
    OPEN buyer_cursor;
    read_loop: LOOP
        SET v_sell_id = NULL;
        SET v_seller_id = NULL;
...
CREATE EVENT process_ticket_exhchanges
ON SCHEDULE EVERY 60 SECOND
DO
BEGIN
    CALL process_pending_buyers();
END//
